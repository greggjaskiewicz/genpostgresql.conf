...
Licensed under PostgreSQL BSD Licence
...
class PostgresConfig(object):
    ...
    _options = {
                'work_mem': 'memory',
                'wal_buffers': 'memory',
                'seq_page_cost': '0.0to50.0',
                'cpu_tuple_cost': '0.0to50.0',
                'shared_buffers': 'memory',
                'random_page_cost': '0.0to50.0',
                'cpu_operator_cost': '0.0to50.0',
                'checkpoint_segments': 'uint',
                'maintenance_work_mem': 'memory',
                'effective_cache_size': 'memory',
                'cpu_index_tuple_cost': '0.0to50.0',
                'default_statistics_target': 'uint',
                'checkpoint_completion_target': '0.0to1.0'
    }
    ...
    def cross_over(self, mate):
        ...
        """
        now that we have randomised newconfig dictionary go through it
        and every second option shall be replaced with mate values
        """
        i = 0
        newconfig = dict()
    ...
    def get_config(self):
        ...
        for option in self.config:
            ...
            if self._options[option] == 'memory':
                memory_val = self._memory_value_from_int(self.config[option])
                config = config + '\n' + option + '\t= ' + memory_val
            else:
                config = config + '\n' + option + '\t= ' + stroption
        return config
